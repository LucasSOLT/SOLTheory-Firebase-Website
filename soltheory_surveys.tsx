            ...
            {survey.questions?.map((q: any, i: number) => (
              <div key={q.id || i} className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <span className="text-xs font-medium text-slate-400 capitalize">{q.type}{q.type === "choice" && q.options ? ` Ã¢â‚¬â€ ${q.options.length} options` : ""}</span>
                </div>
              </div>
            ))}
          </div>
              ...
              <div className="bg-[#fefdfb] rounded-xl border border-slate-200 p-6">
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <h4 className="text-sm font-black text-slate-700 uppercase tracking-widest">
                    Analytics Ã¢â‚¬â€ {responses.length} Response{responses.length !== 1 ? "s" : ""}
                  </h4>
                </div>
                    ...
                    <summary className="flex items-center justify-between px-5 py-3.5 cursor-pointer hover:bg-slate-100 transition-colors">
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-slate-400 font-medium">{resp.participantOrg || "Ã¢â‚¬â€"} Ã‚Â· {formatDateTime(resp.submittedAt)}</p>
                        </div>
                      </div>
                          ...
                          return (
                            <div key={q.id} className="bg-[#fefdfb] rounded-lg p-3 border border-slate-200">
                              ...
                              {typeof val === "number" ? (
                                ...
                                <p className="text-sm font-medium text-slate-700">{val || "Ã¢â‚¬â€"}</p>
                              )}
                            </div>
                          );
                        })}
