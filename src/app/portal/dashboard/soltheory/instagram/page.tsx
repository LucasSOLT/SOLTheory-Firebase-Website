...
import { Instagram, UploadCloud, Image as ImageIcon, Video, X, Sparkles, Clock, Calendar, Send, Loader2, Eye, Trash2, Link2, CheckCircle2, AlertCircle } from "lucide-react";
      ...
      <header className="px-8 py-6 border-b border-slate-200 bg-white sticky top-0 z-10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Instagram</h1>
              <p className="text-sm text-slate-500 font-medium mt-0.5">Create, schedule, and manage your Instagram content</p>
            </div>
          </div>
          {isConnected && (
      ...
      <div className="p-8 max-w-5xl mx-auto w-full">
        ...
        {!isConnected && (
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-10 text-center max-w-lg mx-auto">
            ...
            <h2 className="text-xl font-bold text-slate-900 mb-2">Connect Your Instagram</h2>
            <p className="text-sm text-slate-500 mb-6 leading-relaxed">
              Link your Instagram Business or Creator account to start creating and scheduling posts directly from your dashboard.
            </p>
            <button 
              onClick={() => setIsConnected(true)}
              className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-rose-500 to-fuchsia-600 text-white font-bold rounded-xl hover:opacity-90 transition-opacity shadow-sm cursor-pointer"
            >
            ...
            <div className="mt-6 flex items-start gap-2 text-left bg-amber-50 border border-amber-200 rounded-xl p-4">
              ...
              <p className="text-xs text-amber-700 leading-relaxed">
                Requires an Instagram Business or Creator account connected to a Facebook Page. Personal accounts are not supported by Meta&apos;s API.
              </p>
            </div>
          </div>
        )}
                ...
                <div className="space-y-5">
                      ...
                      <div className="w-full relative rounded-xl border border-slate-200 bg-slate-50 overflow-hidden">
                        ...
                        <div className="p-4 flex items-center gap-4">
                          <div className="w-10 h-10 rounded-lg bg-rose-100 flex items-center justify-center shrink-0">
                            {file.type.startsWith('video') ? <Video className="w-5 h-5 text-rose-600" /> : <ImageIcon className="w-5 h-5 text-rose-600" />}
                          </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button onClick={handleAIGenerate} disabled={isGenerating} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-gradient-to-r from-rose-500 to-fuchsia-600 text-white hover:opacity-90 transition-opacity disabled:opacity-50 cursor-pointer">
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
                ...
                {scheduleDate && scheduleTime && (
                  <div className="mt-4 p-3 rounded-lg bg-rose-50 border border-rose-200">
                    <p className="text-xs font-semibold text-rose-700">AI will auto-post on {new Date(scheduleDate + 'T' + scheduleTime).toLocaleString('en-US', { weekday: 'short', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}</p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
                ...
                <div className="rounded-xl border border-slate-200 overflow-hidden bg-white">
                  ...
                  {preview ? <img src={preview} alt="Preview" className="w-full aspect-square object-cover" /> : <div className="w-full aspect-square bg-slate-100 flex items-center justify-center"><ImageIcon className="w-10 h-10 text-slate-300" /></div>}
                  {caption ? <p className="px-3 py-2 text-xs text-slate-700 leading-relaxed whitespace-pre-wrap line-clamp-4">{caption}</p> : <p className="px-3 py-3 text-xs text-slate-400 text-center italic">Caption preview</p>}
                </div>
              </div>
            </div>
          </div>
