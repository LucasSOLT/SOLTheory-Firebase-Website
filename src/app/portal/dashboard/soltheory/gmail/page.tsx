...
import {
  ...
  ArrowLeft, Pen, X, Filter, Check, Loader2, UserPlus, MailOpen, Menu,
} from "lucide-react";
...
function GmailView({ uid, refreshToken, userEmail, userName, onConnectAccount }: { uid?: string; refreshToken?: string; userEmail?: string; userName?: string; onConnectAccount?: () => void }) {
  ...
  return (
    ...
    <div className="flex flex-col flex-1 min-w-0 relative">
      {/* Top Bar */}
      <div className="flex items-center gap-3 px-4 py-2.5 border-b shrink-0 border-slate-200/80 bg-white dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[14px] font-semibold text-slate-800 dark:text-white">Gmail</span>
        </div>
        <div className="flex-1 max-w-lg mx-4">
          <div className="relative">
            ...
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
              ...
              className="w-full pl-9 pr-4 py-2 rounded-xl text-[13px] outline-none transition-all placeholder:text-slate-400 bg-slate-50 border border-slate-200/80 focus:ring-blue-100 focus:border-blue-300 dark:bg-slate-850 dark:border-slate-700 dark:text-white dark:focus:ring-slate-700 dark:focus:border-slate-600" />
            {searchQuery && (
        ...
        <div className="relative" ref={accountMenuRef}>
          <button onClick={() => setAccountMenuOpen(!accountMenuOpen)}
            className="flex items-center gap-2 px-2 py-1 rounded-full border transition-all cursor-pointer border-slate-200/80 hover:border-slate-300 bg-white dark:border-slate-700 dark:hover:border-slate-600 dark:bg-slate-800 dark:text-slate-350">
            <span className="text-[11px] font-semibold pl-1.5 text-slate-500 dark:text-slate-400">NXT Suite</span>
          ...
          {accountMenuOpen && (
            <>
              ...
              <div className="absolute right-0 top-full mt-2 w-[300px] rounded-2xl border shadow-2xl z-[210] overflow-hidden bg-white border-slate-200 dark:bg-slate-850 dark:border-slate-700">
                <div className="p-5 text-center border-b border-slate-100 dark:border-slate-750">
                  <p className="text-[13px] font-semibold text-slate-800 dark:text-white">{userEmail || "No account connected"}</p>
                  {isConnected && <p className="text-[11px] text-slate-400 mt-0.5">Managed by NXT Suite</p>}
                </div>
                <div className="px-5 py-4 flex flex-col items-center">
                  <div className="w-16 h-16 rounded-full bg-gradient-to-br from-slate-500 to-slate-700 flex items-center justify-center text-white text-xl font-bold mb-2 ring-2 ring-slate-200 ring-offset-2 dark:ring-slate-700 dark:ring-offset-slate-800">
                    {displayInitials}
                  </div>
                  <p className="text-[15px] font-semibold mt-1 text-slate-800 dark:text-white">Hi, {displayName.split(" ")[0]}!</p>
                </div>
                <div className="px-4 pb-3">
                  <button onClick={() => { openCompose("new"); setAccountMenuOpen(false); }}
                    className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-[12px] font-semibold transition-colors cursor-pointer bg-slate-800 text-white hover:bg-slate-900 dark:bg-slate-700 dark:hover:bg-slate-600">
                    <Pen className="w-3 h-3" /> Compose New Email
                  </button>
                </div>
                <div className="border-t px-3 py-3 border-slate-100 dark:border-slate-750">
                  {isConnected && (
                    <div className="flex items-center gap-3 px-2 py-2.5 rounded-xl transition-colors hover:bg-slate-55 dark:hover:bg-slate-800">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[12px] font-semibold truncate text-slate-700 dark:text-slate-200">{displayName}</p>
                  ...
                  <button onClick={() => { onConnectAccount?.(); setAccountMenuOpen(false); }}
                    className="w-full flex items-center gap-3 px-2 py-2.5 rounded-xl transition-colors cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 dark:text-slate-350">
                    <div className="w-8 h-8 rounded-full flex items-center justify-center text-slate-400 shrink-0 bg-slate-100 dark:bg-slate-800"><UserPlus className="w-3.5 h-3.5" /></div>
                    <span className="text-[12px] font-medium text-slate-600 dark:text-slate-300">Add another account</span>
                  </button>
                </div>
                <div className="border-t px-4 py-3 flex items-center justify-center border-slate-100 dark:border-slate-750">
      ...
      <div className="flex flex-1 min-h-0">
        ...
        {showMobileFolders && (
          <div className="fixed inset-0 z-[100] md:hidden" onClick={() => setShowMobileFolders(false)}>
            <div className="absolute inset-0 bg-black/40" />
            <div className="absolute left-0 top-0 bottom-0 w-56 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 shadow-xl py-3" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between px-3 mb-2">
                <span className="text-[13px] font-semibold text-slate-700 dark:text-white">Folders</span>
                <button onClick={() => setShowMobileFolders(false)} className="w-7 h-7 rounded-lg flex items-center justify-center text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer">
                  <X className="w-4 h-4" />
                </button>
              </div>
              <div className="space-y-0.5 px-2">
                {folders.map((f) => {
                  ...
                  return (
                    <button key={f.id} onClick={() => { setActiveFolder(f.id); setSelectedEmail(null); setFilterUnread(false); setShowMobileFolders(false); }}
                      className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-[13px] font-medium transition-colors cursor-pointer ${
                        activeFolder === f.id ? "bg-slate-200/60 text-slate-800 dark:bg-slate-800 dark:text-white" : "text-slate-500 hover:bg-slate-100/60 hover:text-slate-700 dark:text-slate-400 dark:hover:bg-slate-850 dark:hover:text-slate-250"
                      }`}>
                      <f.icon className={`w-4 h-4 ${activeFolder === f.id ? "text-slate-700 dark:text-white" : "text-slate-400 dark:text-slate-500"}`} />
                      <span className="flex-1 text-left">{f.label}</span>
                      {count > 0 && (
                        <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full min-w-[18px] text-center ${
                          activeFolder === f.id ? "bg-slate-300/50 text-slate-600 dark:bg-slate-750 dark:text-white" : "bg-slate-200/60 text-slate-400 dark:bg-slate-800 dark:text-slate-500"
                        }`}>{count}</span>
                      )}
                    </button>
                  );
        ...
        <div className="w-48 border-r hidden md:flex flex-col py-2.5 shrink-0 border-slate-200/80 bg-slate-50/30 dark:border-slate-800 dark:bg-slate-900/40">
          <div className="space-y-0.5 px-2">
            {folders.map((f) => {
              const count = folderCounts[f.id] || 0;
              return (
                <button key={f.id} onClick={() => { setActiveFolder(f.id); setSelectedEmail(null); setFilterUnread(false); }}
                  ...
                  <f.icon className={`w-3.5 h-3.5 ${activeFolder === f.id ? "text-slate-700 dark:text-white" : "text-slate-400 dark:text-slate-500"}`} />
                  <span className="flex-1 text-left">{f.label}</span>
                  {count > 0 && (
                    <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full min-w-[18px] text-center ${
                      activeFolder === f.id ? "bg-slate-300/50 text-slate-600 dark:bg-slate-750 dark:text-white" : "bg-slate-200/60 text-slate-400 dark:bg-slate-800 dark:text-slate-500"
        ...
        {/* Email content */}
        {selectedEmail ? (
          <div className="flex-1 flex flex-col overflow-y-auto bg-white text-slate-800 dark:bg-slate-900 dark:text-slate-200">
            <div className="flex items-center gap-2 px-5 py-3 border-b shrink-0 border-slate-100 bg-white dark:border-slate-800 dark:bg-slate-900/50">
              <button onClick={() => setSelectedEmail(null)} className="w-7 h-7 rounded-lg flex items-center justify-center text-slate-400 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800" title="Back to list">
                <ArrowLeft className="w-4 h-4" />
              </button>
              <div className="flex items-center gap-1 ml-auto">
                <button onClick={() => openCompose("reply", selectedEmail)} className="w-7 h-7 rounded-lg flex items-center justify-center text-slate-400 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800" title="Reply"><Reply className="w-3.5 h-3.5" /></button>
                <button onClick={() => openCompose("replyAll", selectedEmail)} className="w-7 h-7 rounded-lg flex items-center justify-center text-slate-400 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800" title="Reply All"><ReplyAll className="w-3.5 h-3.5" /></button>
                <button onClick={() => openCompose("forward", selectedEmail)} className="w-7 h-7 rounded-lg flex items-center justify-center text-slate-400 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800" title="Forward"><Forward className="w-3.5 h-3.5" /></button>
                <div className="w-px h-4 mx-0.5 bg-slate-200 dark:bg-slate-800" />
                <button onClick={() => archiveEmail(selectedEmail.id)} className="w-7 h-7 rounded-lg flex items-center justify-center text-slate-400 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800" title="Archive"><Archive className="w-3.5 h-3.5" /></button>
                <button onClick={() => deleteEmail(selectedEmail.id)} className="w-7 h-7 rounded-lg flex items-center justify-center text-slate-400 cursor-pointer hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-950/30 dark:hover:text-red-400" title="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
                <button onClick={(ev) => toggleStar(selectedEmail.id, ev)} className="w-7 h-7 rounded-lg flex items-center justify-center cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800" title="Star">
            ...
            <div className="px-8 py-6 max-w-3xl">
              <h1 className="text-lg font-semibold mb-4 leading-snug text-slate-900 dark:text-white">{selectedEmail.subject}</h1>
              <div className="flex items-center gap-3 mb-5">
                <div className="w-9 h-9 rounded-full flex items-center justify-center font-semibold text-[11px] bg-slate-200 text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                  {selectedEmail.from.split(" ").map((n) => n[0]).join("").slice(0, 2).toUpperCase()}
                </div>
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className="text-[13px] font-semibold text-slate-800 dark:text-slate-200">{selectedEmail.from}</span>
              ...
              <div className="text-[13px] leading-relaxed whitespace-pre-wrap text-slate-655 dark:text-slate-300" dangerouslySetInnerHTML={{ __html: selectedEmail.body }} />
              {selectedEmail.hasAttachment && (
                <div className="mt-5 p-3 rounded-xl border flex items-center gap-3 border-slate-200/80 bg-slate-50/50 dark:border-slate-800 dark:bg-slate-850/50">
                  <Paperclip className="w-4 h-4 text-slate-400" />
                  <span className="text-[12px] font-medium text-slate-500">1 Attachment</span>
                  <button className="ml-auto text-[11px] font-semibold cursor-pointer text-slate-600 hover:text-slate-800 dark:text-slate-300 dark:hover:text-white">Download</button>
                </div>
              )}
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800">
                <button onClick={() => openCompose("reply", selectedEmail)}
                  className="flex items-center gap-2 px-4 py-2.5 border rounded-xl text-[12px] transition-colors cursor-pointer border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-700 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">
          ...
          <div className="flex-1 flex flex-col bg-white dark:bg-slate-900">
            <div className="flex items-center justify-between px-4 py-2 border-b shrink-0 border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-1.5">
                {/* Mobile-only Folders toggle */}
                <button onClick={() => setShowMobileFolders(true)}
                  className="w-7 h-7 rounded-lg flex items-center justify-center text-slate-400 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 md:hidden" title="Folders">
                  <Menu className="w-4 h-4" />
                </button>
                <button onClick={markAllAsRead} className="w-7 h-7 rounded-lg flex items-center justify-center text-slate-400 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800" title="Mark all as read">
                  <RefreshCw className="w-3.5 h-3.5" />
                </button>
                <button onClick={() => setFilterUnread((v) => !v)}
                  className={`w-7 h-7 rounded-lg flex items-center justify-center cursor-pointer transition-colors ${filterUnread ? "bg-blue-50 text-blue-500" : "hover:bg-slate-100 text-slate-400 dark:hover:bg-slate-800 dark:text-slate-500"}`} title={filterUnread ? "Show all" : "Show unread only"}>
            ...
            <div className="flex-1 overflow-y-auto relative">
              ...
              {filteredEmails.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center px-8 py-20">
                  <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-4 bg-slate-100 dark:bg-slate-850">
                ...
                filteredEmails.map((email) => {
                  ...
                  <div key={email.id} onClick={() => {
                    ...
                    className={`flex items-center gap-3 px-4 py-3 border-b cursor-pointer transition-all duration-300 group border-slate-100/80 dark:border-slate-850 ${!email.read ? "bg-blue-50/40 border-l-[3px] border-l-blue-500 dark:bg-slate-850/30" : "opacity-60 border-l-[3px] border-l-transparent"} hover:bg-slate-50 hover:!opacity-100 dark:hover:bg-slate-800/60 ${isCleanupTarget ? "!opacity-100 ring-2 ring-red-400/70 bg-red-50/50 shadow-sm" : isHighlighted ? "ring-2 ring-blue-400/60 bg-blue-50/40 shadow-sm" : ""} ${isSelected ? "!opacity-100 ring-2 ring-purple-400/60 bg-purple-50/40 shadow-sm" : ""}`}>
                    {emailSelectMode && (
                    ...
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center font-bold text-[9px] shrink-0 ${!email.read ? "bg-blue-100 text-blue-600" : "bg-slate-200 text-slate-500 dark:bg-slate-800 dark:text-slate-400"}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className={`text-[12px] truncate ${!email.read ? "font-bold text-slate-900 dark:text-white" : "font-normal text-slate-400 dark:text-slate-500"}`}>{email.from}</span>
                        {email.folder === "sent" && <span className="text-[10px] text-slate-500 shrink-0">→ {email.to || email.toEmail}</span>}
                      </div>
                      <div className="flex items-center gap-1.5">
                        <span className={`text-[12px] truncate ${!email.read ? "font-semibold text-slate-700 dark:text-slate-300" : "text-slate-400"}`}>{email.subject}</span>
                        <span className="text-[11px] truncate flex-1 text-slate-400 dark:text-slate-500">— {email.preview}</span>
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-0.5 shrink-0 ml-1">
                      <span className={`text-[11px] whitespace-nowrap ${!email.read ? "font-bold text-slate-700 dark:text-slate-300" : "text-slate-400"}`}>{email.date}</span>
                      <span className={`text-[10px] whitespace-nowrap ${!email.read ? "text-slate-500 dark:text-slate-400" : "text-slate-300 dark:text-slate-500"}`}>{email.time}</span>
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                      {!email.read && (
                        <button
                          onClick={(ev) => markAsRead(email.id, ev)}
                          className="w-6 h-6 rounded-md flex items-center justify-center opacity-0 group-hover:opacity-100 hover:bg-blue-50 text-slate-400 hover:text-blue-500 transition-all cursor-pointer dark:hover:bg-slate-800"
                          title="Mark as read"
                        >
      ...
      {/* Compose Modal */}
      {composeOpen && (
        <div className="absolute bottom-4 left-2 right-2 md:left-4 md:right-auto md:w-[500px] rounded-2xl shadow-2xl border flex flex-col z-50 overflow-hidden bg-white border-slate-200 dark:bg-slate-850 dark:border-slate-700">
          ...
          <div className="p-3 space-y-2">
            <div className="flex items-center gap-2 border-b pb-2 border-slate-100 dark:border-slate-700">
              <span className="text-[12px] text-slate-400 w-8 shrink-0">To</span>
              <input type="text" placeholder="Recipients" value={compose.to} onChange={(e) => setCompose((s) => ({ ...s, to: e.target.value }))} className="flex-1 text-[13px] outline-none text-slate-700 placeholder:text-slate-300 dark:bg-transparent dark:text-white dark:placeholder:text-slate-500" />
            </div>
            <div className="flex items-center gap-2 border-b pb-2 border-slate-100 dark:border-slate-700">
              <span className="text-[12px] text-slate-400 w-8 shrink-0">Cc</span>
              <input type="text" value={compose.cc} onChange={(e) => setCompose((s) => ({ ...s, cc: e.target.value }))} className="flex-1 text-[13px] outline-none text-slate-700 placeholder:text-slate-300 dark:bg-transparent dark:text-white dark:placeholder:text-slate-500" />
            </div>
            <div className="flex items-center gap-2 border-b pb-2 border-slate-100 dark:border-slate-700">
              <span className="text-[12px] text-slate-400 w-8 shrink-0">Subj</span>
              <input type="text" placeholder="Subject" value={compose.subject} onChange={(e) => setCompose((s) => ({ ...s, subject: e.target.value }))} className="flex-1 text-[13px] outline-none text-slate-700 placeholder:text-slate-300 dark:bg-transparent dark:text-white dark:placeholder:text-slate-500" />
            </div>
            <textarea ref={bodyRef} placeholder="Write your message..." value={compose.body} onChange={(e) => setCompose((s) => ({ ...s, body: e.target.value }))}
              className="w-full h-44 text-[13px] outline-none resize-none leading-relaxed p-1 text-slate-600 placeholder:text-slate-300 dark:bg-transparent dark:text-white dark:placeholder:text-slate-550" />
          </div>
          <AIComposeAssist
            subject={compose.subject}
            body={compose.body}
          ...
          <div className="flex items-center justify-between px-4 py-2.5 border-t border-slate-100 dark:border-slate-700">
            <div className="flex items-center gap-2">
              <button onClick={handleSend} disabled={!compose.to.trim() || !compose.subject.trim() || sendingEmail}
                className={`px-5 py-2 rounded-xl text-[12px] font-semibold transition-colors cursor-pointer flex items-center gap-1.5 ${
                  compose.to.trim() && compose.subject.trim() && !sendingEmail 
                    ? "bg-slate-800 text-white hover:bg-slate-900 dark:bg-slate-200 dark:text-black dark:hover:bg-slate-350" 
              ...
              <button className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800" title="Attach file"><Paperclip className="w-4 h-4" /></button>
            </div>
            <button onClick={() => { setComposeOpen(false); setCompose({ to: "", cc: "", subject: "", body: "", mode: "new" }); }} className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 cursor-pointer hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-950/30 dark:hover:text-red-400" title="Discard">
...
export default function GmailPage() {
  ...
  return (
    <div className="w-full h-full rounded-2xl overflow-hidden shadow-sm border relative bg-white border-slate-200/80 dark:bg-slate-900 dark:border-slate-800" style={{ WebkitFontSmoothing: "antialiased", MozOsxFontSmoothing: "grayscale" } as React.CSSProperties}>
      <GmailView
