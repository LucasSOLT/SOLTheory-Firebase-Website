...
function ActionBoardContent() {
  ...
  // Derive org from URL path – /portal/dashboard/${orgId}/... vs /portal/dashboard/${orgId}/...
  const ORG_ID = orgId;
  ...
  useEffect(() => {
    ...
    const fetchMembers = async () => {
      try {
        ...
        // Also fetch external contacts for this org
        try {
          const externalSnap = await getDocs(collection(firestore, `orgs/${ORG_ID}/external-contacts`));
          ...
          console.warn("[ActionBoard] Failed to fetch external contacts:", extErr);
        }
  ...
  const addTask = async () => {
    ...
    const assigneeMember = orgMembers.find(m => m.uid === assigneeUid) || orgMembers.find(m => m.email === assigneeUid);
    const isSelfAssign = assigneeUid === user.uid;
    const isExternal = assigneeUid.includes('@') && !orgMembers.some(m => m.uid === assigneeUid) && !orgMembers.some(m => m.email === assigneeUid);

    if (isExternal) {
      try {
        const extRef = doc(firestore, `orgs/${ORG_ID}/external-contacts`, assigneeUid);
        await setDoc(extRef, { email: assigneeUid, name: assigneeUid, addedAt: serverTimestamp(), addedBy: user.uid }, { merge: true });
      } catch (err) {
        console.warn("[ActionBoard] Failed to save external contact", err);
      }
    }
    ...
    else if (isExternal) status = "direct";
    ...
    const taskData: Record<string, any> = {
      ...
      assignedToEmail: isSelfAssign ? (user.email || "") : (assigneeMember?.email || (isExternal ? assigneeUid : "")),
      assignedToName: isSelfAssign ? (user.displayName || "") : (assigneeMember?.displayName || (isExternal ? assigneeUid : "")),
  ...
  if (isLoading) {
    return (
      <div className={`flex items-center justify-center h-full ${isDarkMode ? 'bg-slate-900' : 'bg-[#f5f1e8]'}`}>
              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  <div className={`max-h-36 overflow-y-auto space-y-1 rounded-xl border ${isDarkMode ? 'border-slate-700 bg-slate-900' : 'border-slate-100 bg-white'} p-1`}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {m.isExternal && (
                            <span className="text-[9px] text-amber-500 font-bold flex items-center gap-1 mt-0.5">
                              <AlertTriangle className="w-3 h-3" /> External Recipient
                            </span>
                          )}
                        </div>
                    ...
                    {/* Allow external email assignment if valid email and not found */}
                    {assigneeSearch.trim().includes('@') && !orgMembers.some(m => m.email.toLowerCase() === assigneeSearch.trim().toLowerCase()) && (
                      <button onClick={() => setNewAssignee(assigneeSearch.trim())} className={`w-full text-left px-3 py-2 rounded-lg flex items-center gap-2.5 transition-colors ${newAssignee === assigneeSearch.trim() ? (isDarkMode ? "bg-amber-950/80 ring-1 ring-amber-800 text-amber-100" : "bg-amber-50 ring-1 ring-amber-300 text-amber-900") : (isDarkMode ? "bg-amber-900/30 hover:bg-amber-900/50 text-amber-200" : "bg-amber-50 hover:bg-amber-100 text-amber-800")}`}>
                        <div className="w-6 h-6 rounded-full bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white text-[9px] font-bold"><Mail className="w-3 h-3" /></div>
                        <div className="flex-1 min-w-0">
                          <span className="text-sm font-medium truncate block">Assign to {assigneeSearch.trim()}</span>
                          <span className="text-[9px] font-bold flex items-center gap-1 mt-0.5">
                            <AlertTriangle className="w-3 h-3" /> External Recipient
                          </span>
                        </div>
                        {newAssignee === assigneeSearch.trim() && <CheckCircle2 className="w-4 h-4 text-amber-500 shrink-0" />}
                      </button>
                    )}
                  </div>
              ...
              {archivedTasks.length > 0 && (
                <>
                  ...
                  {archivedTasks.map(task => (
                    <div key={task.id} className={`${isDarkMode ? 'bg-slate-800 border-emerald-900 text-white' : 'bg-[#faf8f3] border-emerald-200'} border rounded-xl p-4 shadow-sm`}>
                      <div className="flex items-center gap-2 mb-2">
                        <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md ${PRIORITY_STYLES[task.priority]}`}>{task.priority}</span>
                          ...
                          {isAdmin ? (
                            <>
                              ...
                              {restoreDropdownId === task.id && (
                                <div className={`absolute left-0 bottom-full mb-1 w-52 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-[#faf8f3] border-slate-200'} border rounded-xl shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-150`}>
                                  <button onClick={() => restoreTask(task.id)} className={`w-full text-left px-3 py-2 text-xs ${isDarkMode ? 'text-slate-300 hover:bg-slate-800' : 'text-slate-600 hover:bg-[#f2ece0]'} transition-colors flex items-center gap-2`}>
                                  ...
                                  {orgMembers.map(m => (
                                    <button key={m.uid} onClick={() => restoreTask(task.id, m.uid, m.email, m.displayName || m.email)} className={`w-full text-left px-3 py-2 text-xs ${isDarkMode ? 'text-slate-300 hover:bg-slate-800' : 'text-slate-600 hover:bg-[#f2ece0]'} transition-colors flex items-center gap-2`}>
              ...
              {deniedTasks.length > 0 && (
                <>
                  ...
                  {deniedTasks.map(task => (
                    <div key={task.id} className={`${isDarkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-[#faf8f3] border-slate-200'} border rounded-xl p-4 shadow-sm opacity-70`}>
                      <div className="flex items-center gap-2 mb-2">
                        <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md ${PRIORITY_STYLES[task.priority]}`}>{task.priority}</span>
