...
export default function CRMPage() {
  ...
  const [rowHeight, setRowHeight] = useState<number>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('crm_row_height');
      if (saved) return parseInt(saved, 10);
    }
    return 48; // default
  });
  const rowResizingRef = useRef<{ startY: number, startHeight: number } | null>(null);
  ...
  const handleRowResizeMouseDown = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    rowResizingRef.current = { startY: e.clientY, startHeight: rowHeight };
    ...
    const onMouseMove = (ev: MouseEvent) => {
      ...
      const newHeight = Math.max(32, startHeight + diff); // Min 32px
      setRowHeight(newHeight);
      localStorage.setItem('crm_row_height', newHeight.toString());
    };
    const onMouseUp = () => {
    ...
    return (
      <div className={`flex items-center justify-center min-h-screen ${isDarkMode ? 'bg-slate-950 text-white' : 'bg-[#F9FAFB]'}`}>
        <div className={`rounded-2xl border shadow-xl p-10 max-w-md w-full mx-4 text-center space-y-6 ${isDarkMode ? 'bg-slate-900 border-slate-850 text-white' : 'bg-[#faf8f3] border-[#E5E7EB]'}`}>
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-800 tracking-tight">{getOrgLabel(orgId)} Contacts</h1>
                          ...
                          >
                            <td className={`sticky left-0 z-10 px-3 py-3 border-r ${isDarkMode ? (idx % 2 === 1 ? 'bg-slate-800/20' : 'bg-slate-900') + ' border-slate-700' : (idx % 2 === 1 ? 'bg-slate-50/50' : 'bg-white') + ' border-slate-200'} group-hover:${isDarkMode ? 'bg-slate-800/50' : 'bg-indigo-50/30'}`} onClick={(e) => e.stopPropagation()}>
                              ...
                              <div
                                ...
                                onClick={(e) => e.stopPropagation()}
                                className="absolute bottom-0 left-0 w-full h-[4px] cursor-row-resize z-20 hover:bg-indigo-400/40 active:bg-indigo-500/60"
                              />
                            </td>
                              ...
                              switch (field.type) {
                                      ...
                                      {tagsArr.length > 0 ? (
                                        <div className="flex flex-nowrap gap-1 overflow-hidden">
                                          {tagsArr.slice(0, 2).map((tag: string) => (
                                            <span key={tag} className={`text-[9px] font-semibold px-2 py-0.5 rounded-full border truncate whitespace-nowrap shrink-0 ${getTagStyles(tag)}`}>{tag}</span>
                                          ))}
                                          {tagsArr.length > 2 && <span className="text-[9px] font-medium text-slate-400 shrink-0">+{tagsArr.length - 2}</span>}
                                        </div>
                                      ) : dash}
                                    </td>
                                  );
                                ...
                                default: {
                                  ...
                                  if (field.id === "firstName") {
                                    return (
                                      <td key={field.id} className={`px-3 py-3 border-r font-medium ${isDarkMode ? 'text-white border-slate-700' : 'text-slate-800 border-slate-200'}`} style={columnWidths[field.id] ? { width: columnWidths[field.id], minWidth: columnWidths[field.id], maxWidth: columnWidths[field.id], overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } : { maxWidth: '250px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                        <div className="flex flex-col items-start overflow-hidden">
                                          <span className="truncate w-full block">{val ? String(val) : dash}</span>
