...
export default function AIKnowledgeBasePage() {
  ...
  const fetchRAGDocs = async () => {
    ...
    try {
      ...
      const q = query(collection(firestore, "users", user.uid, "agents", `${orgId}_${agentId}`, "knowledge_docs"), orderBy("createdAt", "desc"));
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-8 py-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href={`/portal/dashboard/${orgId}/ai-agents/jarvis`}>
        ...
        <div className={`border rounded-2xl overflow-hidden ${cardBg}`}>
          ...
          <div className={`px-6 py-3 ${isDarkMode ? 'bg-slate-800/50 border-t border-slate-800' : 'bg-[#faf8f3]/50 border-t border-slate-100'}`}>
            <p className={`text-xs text-center ${textSecondary}`}>
              ...
              {activeSettingsTab === "pact" && "Facts your agents have learned about you â€” automatically extracted from conversations."}
            </p>
          </div>
        </div>
                        ...
                        <Button variant="ghost" size="icon" className="text-slate-400 hover:text-red-400" onClick={async () => {
                          if (confirm('Delete this knowledge entry?')) {
                            try {
                              ...
                              await deleteDoc(firestoreDoc(firestore!, "users", user!.uid, "agents", `${orgId}_${agentId}`, "knowledge_docs", ragDoc.id));
                              try {
                                const chunksRef = collection(firestore!, "users", user!.uid, "agents", `${orgId}_${agentId}`, "knowledge_chunks");
