...
export default function AgenticProspectingPage() {
  ...
  const supportEmail = getOrgConfig(orgPrefix)?.adminEmails?.[0] || getOrgConfig(orgPrefix)?.supportEmail || "support@soltheory.com";
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await sendEmail(user.uid, refreshToken, supportEmail, consultSubject, consultBody.replace(/\n/g, "<br/>"));
      if (res.success) {
        if (firestore) {
          ...
          await addDoc(collection(firestore, "support_tickets"), {
            ...
            toEmail: supportEmail, toName: "Support",
  ...
  const gmailUrl = user?.uid ? `/api/auth/google?uid=${user.uid}&agentId=campaigning&origin=${orgPrefix}&returnTo=agentic-prospecting` : "#";
                ...
                {sendResult === "success" ? (
                  <div className="text-center py-8">
                    ...
                    <p className={`text-[13px] max-w-sm mx-auto ${txt2}`}>
                      Your message has been sent to <span className="font-medium">{supportEmail}</span>. Expect a follow-up within 1–2 business days.
                    </p>
