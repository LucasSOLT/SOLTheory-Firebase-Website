...
export default function PhilanthropicGrantScoutDashboard() {
  ...
  } = useGrantSessions(orgPrefix);
  const { orgProfile, saveOrgProfile } = useOrgProfile(orgPrefix);
  ...
  const philanthropicSessions = useMemo(
    () => sessions.filter((s: any) => s.searchMode === "philanthropic"),
    [sessions]
  );
  ...
  useEffect(() => {
    if (!firestore) return;
    const q = query(collection(firestore, "grant_suggestions"), where("orgId", "==", orgPrefix));
    const unsub = onSnapshot(q, s => {
      const all = s.docs.map(d => ({ id: d.id, ...d.data() } as GrantRecord));
      const philanthropicSessionIds = new Set(philanthropicSessions.map(s => s.id));
      const filtered = activeSessionId
        ? all.filter((g: any) => g.sessionId === activeSessionId)
        : all.filter((g: any) => g.searchMode === 'philanthropic' || philanthropicSessionIds.has(g.sessionId));
      setGrants(filtered);
      setGrantsLoading(false);
    }, () => setGrantsLoading(false));
    return unsub;
  }, [firestore, activeSessionId, philanthropicSessions]);
  ...
  return (
    <div className={`min-h-screen ${surface2} ${txt} overflow-y-auto`} style={{ fontFamily: "'Inter', system-ui, -apple-system, sans-serif" }}>
      ...
      <div className="max-w-[1280px] mx-auto px-6 md:px-10 py-8">
        ...
        <div className="flex items-center gap-2.5 mb-6 px-4 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 via-orange-500 to-rose-500 shadow-sm">
          <Heart className="w-4 h-4 text-white/90 drop-shadow-sm" />
          <span className="text-[11px] font-bold text-white tracking-wide uppercase">Philanthropic Grant Scout</span>
          <span className="text-[10px] text-white/70 font-medium ml-auto">Foundations · Corporate Giving · IRS 990</span>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
          <div>
            <div className="flex items-center gap-2.5 mb-1">
              <h1 className={`text-2xl font-semibold tracking-tight ${txt}`}>Philanthropic Grant Scout</h1>

            </div>
            <p className={`text-[13px] leading-relaxed max-w-xl ${txt2}`}>
              Discover foundations, corporate giving programs, and philanthropic organizations that fund work like yours.
            </p>
          </div>
          <button
        ...
        <div className={`grid grid-cols-2 md:grid-cols-4 border rounded-xl overflow-hidden mb-8 ${surface} ${divide}`}>
          {[
            { label: "Sessions active", value: String(philanthropicSessions.length) },
        ...
        <div className={`rounded-xl border p-4 mb-6 ${surface}`}>
          <SessionSwitcher
            sessions={philanthropicSessions}
            ...
            onDuplicateSession={async (id) => {
              ...
              if (!src) return;
              await createSession(src.name + " (Copy)", src.config, undefined, "philanthropic");
            }}
              ...
              {tags.map((tag, i) => (
                <span
                  ...
                  className={`inline-flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-1 rounded-lg transition-colors ${
                    dk ? "bg-[#1a1b1f] text-slate-400 border border-[#252730]" : "bg-amber-50 text-slate-600 border border-amber-200"
                  }`}
                >
                  {tag.icon}
                  {tag.text}
        ...
        {grantsLoading ? (
          ...
          <div className={`rounded-xl border overflow-hidden mb-10 ${surface}`}>
            ...
            <div className={`grid grid-cols-12 gap-3 px-5 py-2.5 text-[11px] font-medium uppercase tracking-wide border-b ${dk ? "bg-[#0c0d0f] text-slate-500 border-[#1e2028]" : "bg-[#f7f7f5] text-slate-400 border-slate-200"}`}>
              ...
              <div className="col-span-3">Funder</div>
        ...
        <div className={`flex items-center justify-between py-4 border-t ${divide}`}>
          <p className={`text-[11px] ${txt3}`}>
            Scanning foundation databases, IRS 990 filings, and corporate giving programs to find philanthropic funding opportunities.
          </p>
        </div>
      </div>
      ...
      {showNewSessionWizard && (
        <GrantAgentConfigModal
          ...
          onSave={async (config) => {
            ...
            await createSession(sessionName, config, undefined, "philanthropic");
