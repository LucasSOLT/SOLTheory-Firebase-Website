...
import { getOrgLabel, isOrgAdmin } from "@/lib/org-config";
import {
  collection,
  addDoc,
  getDocs,
...
function getWeekBounds(date: Date) {
  ...
  const label = `Week of ${monthDay(monday)} â€“ ${monthDay(sunday)}, ${sunday.getFullYear()}`;
...
export default function SurveysPage() {
  ...
  // Derived
  const isAdmin = isOrgAdmin(orgId, user?.email || "");
  ...
  const [editDescription, setEditDescription] = useState(
    `Help us improve the ${getOrgLabel(orgId)} dashboard experience. Your answers are confidential and reviewed only by the SOL Theory admin team.`
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!isOrgAdmin(orgId, user.email || "")) {
          const rq = query(
  ...
  const saveSurvey = useCallback(async () => {
    ...
    const docId = `${orgId}_${selectedWeek}`;
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-[#f5f1e8] -mx-4 -mb-4 md:-mx-10 md:-mb-10">
        <div className="text-center space-y-4 animate-in fade-in duration-300">
          ...
          <p className="text-sm text-slate-400">Loading surveysâ€¦</p>
        </div>
      </div>
    );
  }
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Describe the survey purposeâ€¦"
              />
            </div>
          </div>
                    ...
                    <div className="flex-1 space-y-3">
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        >
                          ...
                          <option value="text">âœï¸ Text</option>
                        </select>
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-slate-400 mt-1">
                  ...
                  Â· {surveyQuestions.length} questions Â·{" "}
        ...
        <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-300">
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-sm text-slate-400">
                Create and manage weekly surveys for {getOrgLabel(orgId)} dashboard users.
              </p>
            </div>
        ...
        <div className="w-full max-w-2xl space-y-8">
          ...
          {!question.required && (
            <p className="text-center text-xs text-slate-400">
              This question is optional â€” you can skip it.
            </p>
          )}
        </div>
      </div>
