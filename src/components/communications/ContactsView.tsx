...
export function ContactsView() {
  ...
  return (
    <div className={`flex h-full w-full rounded-3xl overflow-hidden border shadow-sm flex-col ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`}>
      <div className={`h-16 px-6 border-b flex items-center gap-3 shrink-0 ${isDarkMode ? 'border-slate-700 bg-slate-800' : 'border-slate-200 bg-slate-50'}`}>
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Left pane: Add Form */}
        <div className={`w-80 border-r p-6 shrink-0 flex flex-col relative z-10 ${isDarkMode ? 'border-slate-700 bg-slate-800/50' : 'border-slate-100 bg-white'}`}>
        ...
        <div className={`flex-1 overflow-y-auto p-6 ${isDarkMode ? 'bg-slate-900/50' : 'bg-slate-50/50'}`}>
           {contacts.length === 0 ? (
             <div className="h-full flex flex-col items-center justify-center text-center opacity-70">
                <div className={`w-20 h-20 rounded-3xl shadow-sm flex items-center justify-center border rotate-12 transition-transform hover:rotate-0 duration-300 mb-6 ${isDarkMode ? 'bg-slate-800 border-slate-700 text-slate-500' : 'bg-white border-slate-100 text-slate-300'}`}>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
               {contacts.map((contact) => (
                 <div key={contact.id} className={`rounded-2xl p-4 shadow-sm flex flex-col relative group transition-all ${isDarkMode ? 'bg-slate-800 border border-slate-700 hover:border-indigo-700 hover:shadow-md' : 'bg-white border border-slate-200 hover:border-indigo-200 hover:shadow-md'}`}>
